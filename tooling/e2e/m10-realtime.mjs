// M10 realtime probe: two clients (Dinda + Maya) share one room through the
// local relay. Asserts REALTIME_PROTOCOL §20 steps 1-9: joint join, mutual
// visibility, Dinda moves, Maya sees smooth interpolated movement, Dinda
// waves, Maya sees the emote, Maya disconnects, Dinda drops her, and the
// local quest still completes.
// Prints M10 REALTIME VERIFIED only when every assertion passes.
...
async function main() {
  ...
  try {
    ...
    // 1-2: joint join, mutual visibility by canonical name
    ...
    // 3-4: Dinda moves; Maya samples smooth interpolated positions
    ...
    let emoteSeen = null;
    {
      ...
      for (;;) {
        const n = await net(maya.page);
        emoteSeen = n.remoteEmotes[dindaIdOnMaya] ?? null;
        if (emoteSeen) break;
        if (Date.now() - start > 8000) fail("Maya never saw the emote");
        await sleep(300);
      }
    }
    if (!["wave", "heart", "celebrate", "laugh", "blessing"].includes(emoteSeen)) {
      fail(`emote outside allowlist: ${emoteSeen}`);
    }
    await maya.page.screenshot({ path: join(ROOT, "docs/qa/m10-duo-390.png") });

    // 7-8: Maya disconnects; Dinda drops her
    ...
    // 9: local quest continues while online
  ...
  console.log("M10 REALTIME VERIFIED");
}
