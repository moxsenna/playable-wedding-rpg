...
try {
  ...
  // --- guests + tokens (project-scoped) ---
  const gs = guests.createGuestStore();
  let r = guests.registerGuest(gs, "demo-ayu-bima", "  Dinda  ", NOW);
  ...
  ok(r.guest.projectId === "demo-ayu-bima", "guest carries project");
  const token = r.guest.token;
  ok(!guests.registerGuest(gs, "demo-ayu-bima", "   ", NOW).ok, "blank name rejected");
  ok(!guests.registerGuest(gs, "No Such Project!", "X", NOW).ok, "bad project rejected");
  ...
  ok(guests.listGuests(gs, "demo-ayu-bima").length === 1, "project listing scoped");

  // --- rsvp upsert per token ---
  const rs = rsvp.createRsvpStore();
  const PID = "demo-ayu-bima";
  let s = rsvp.submitRsvp(gs, rs, { token, projectId: PID, name: "Dinda", attending: "hadir", partySize: 2 }, NOW);
  ok(s.ok && s.created && s.record.partySize === 2, "rsvp created");
  s = rsvp.submitRsvp(gs, rs, { token, projectId: PID, name: "Dinda", attending: "tidak", partySize: 1 }, NOW + 1);
  ok(s.ok && !s.created && s.record.attending === "tidak" && rs.records.length === 1, "rsvp upserted per token");
  ok(!rsvp.submitRsvp(gs, rs, { token: "gt_nope", projectId: PID, name: "X", attending: "hadir", partySize: 1 }, NOW).ok, "rsvp with unknown token rejected");
  ok(!rsvp.submitRsvp(gs, rs, { token, projectId: "raka-naya", name: "Dinda", attending: "hadir", partySize: 1 }, NOW).ok, "cross-project rsvp rejected");
  ok(!rsvp.submitRsvp(gs, rs, { token, projectId: PID, name: "Dinda", attending: "hadir", partySize: 9 }, NOW).ok, "partySize 9 rejected");
  ok(!rsvp.submitRsvp(gs, rs, { token, projectId: PID, name: "", attending: "hadir", partySize: 1 }, NOW).ok, "blank rsvp name rejected");

  // --- guestbook ---
  const gb = guestbook.createGuestbookStore();
  let g = guestbook.addGuestbookEntry(gb, PID, { name: "Maya", message: "Bahagia selalu!" }, NOW);
  ok(g.ok && g.entry.id === "gb-1", "guestbook entry accepted");
  ok(!guestbook.addGuestbookEntry(gb, PID, { name: "M", message: "x".repeat(281) }, NOW).ok, "281-char message rejected");
  ok(!guestbook.addGuestbookEntry(gb, PID, { name: "M", message: "lihat http://spam.example" }, NOW).ok, "link rejected");
  ...
  let d = pub.createDraft(vs, PID, "demo-ayu-bima-v1", snapshot, NOW);
  ...
  ok(!pub.createDraft(vs, PID, "demo-ayu-bima-v1", { ...snapshot, events: [] }).ok, "invalid snapshot never drafts");
  ...
  ok(pub.activeVersion(vs, PID, "demo-ayu-bima-v1")?.id === draftId, "active pinned");
  d = pub.createDraft(vs, PID, "demo-ayu-bima-v1", snapshot, NOW + 1);
  pub.publishDraft(vs, d.version.id);
  pub.activateVersion(vs, d.version.id);
  ok(pub.activeVersion(vs, PID, "demo-ayu-bima-v1")?.version === 2, "v2 supersedes");
  ...
  const ev = audit.recordAudit(as, { projectId: PID, actor: "admin", action: "publish", detail: draftId }, NOW);
