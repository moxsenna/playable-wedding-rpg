// Verifies M8 build integrity: contracts + wedding-core + game + web
// typecheck, then the web production build (contracts feed every consumer).
// Prints M8 BUILD VERIFIED only when every step exits 0.
...
const fail = (msg) => { console.error(`M8 build check FAILED: ${msg}`); process.exit(1); };
...
for (const pkg of ["packages/contracts", "packages/wedding-core", "packages/game", "apps/web"]) {
